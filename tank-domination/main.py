...
while game_running_flag:
    ...
    if 0 <= game_characteristic["player"]["position"]["x"] + position_delta and game_characteristic["player"]["position"]["x"] + position_delta + game_tank_sprite.get_width() <= game_characteristic["cpu"]["position"]["x"]:
        game_characteristic["player"]["position"]["x"] += position_delta    

    # Running the game.
    game_display.fill(game_characteristic["sky"]["color"])

    # Create grass.
    pygame.draw.rect(game_display, game_characteristic["grass"]["color"], pygame.Rect(0, game_characteristic["grass"]["position"]["y"], monitor_display[0], monitor_display[1] - game_characteristic["grass"]["position"]["y"]))

    # Create player and computer.
    game_tank_sprite_player = game_tank_sprite

    game_display.blit(game_tank_sprite_player, (game_characteristic["player"]["position"]["x"], game_characteristic["grass"]["position"]["y"] - game_tank_sprite_player.get_height()))
